...
Date: 2026-05-07
Slug: cypher
Summary: An in-browser, zero-trust, variant-aware extractor for OCCMs, Hard-Time lists and Life-Limited-Parts records. Coming soon from Church Bay Consulting.
Status: published
...
Cypher does not. Every operator gets its own **sealed variant module** — a single Python file that pins the column schema, the validation rules, and the parser logic. When a new document arrives, it is fingerprinted from the first three pages and routed to exactly the same parser that handled the previous documents from that operator. Same input shape → same output shape. Every. Single. Time.
...
Cypher launches publicly soon. If you would like a private walkthrough, [get in touch]({filename}contact.md).

— *Daniel Burke, Church Bay Consulting*

---

*Cypher is an open-source project under the MIT license. The deployed site is hosted on GitHub Pages with no analytics, no cookies, and no server. Built with Pyodide, pdfplumber, pdfminer.six, and the goodwill of every aviation engineer who has ever lost a Saturday to manual data entry.*
