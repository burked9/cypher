...
Confirmed on a small set of A320-232 airframes in this registration cluster
(real-corpus triage). Page 1 header::

    MODEL : A320-232 As of Date : <DATE>
    REG. NO. : <REG> HOURS : <HOURS>
    SERIAL NO. : <SN> CYCLES : <CYCLES>
    AIRCRAFT COMPONENTS LIST
    ATA POSITION DESCRIPTION VENDOR P/N. SERIAL NO. INST_DATE TSN CSN

The header advertises 7 row columns, but across the confirmed files
SERIAL NO./INST_DATE/TSN/CSN are never once populated — every row has only
ATA, POSITION, DESCRIPTION and a trailing PART_NUMBER. Hence
...
A third file triaged into this same header cluster turned out NOT to share
this row shape (its header instead reads `ATA LOCATION NOMENCLATURE VENDOR
PART NO SERIAL NUMBER DATE-INST. REMARK`, 2-digit ATA, extra REMARK column)
-- and is unparseable regardless: pdfminer raises `AttributeError:
'PSKeyword' object has no attribute 'decode'` on a subset of its pages (a
broken embedded CID font, triggered by Chinese-language REMARK values), and
even the pages that don't raise decode every character to the wrong glyph
(verified byte-for-byte identical garbage across independent fresh-session
reads -- not cache poisoning from the crashing pages). That's a
pdfminer/pdfplumber limitation on that specific file, not a parsing-logic
gap, so it's deliberately left out of this variant's scope. The per-page
try/except below exists solely so a future file sharing this cluster's
header but hitting the same font bug on a handful of pages loses only those
...
_OVERRIDES = {
    ...
    # ("200RH*" -- confirmed on a real file, meaning unclear, kept as-is).
