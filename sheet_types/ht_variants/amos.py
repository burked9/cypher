...
A sizeable share of the HT corpus matched on the page-1 signature
`Aircraft Equipment List Report`. Smoke-tested on a handful of
representative files, with row counts ranging from tens to several
hundred per file, no parse changes required.
"""
...
SIGNATURES = [
    ...
    # Hyphenated variant of the same report, used by several operators
    # across the corpus. Identical row layout but the title carries
    # hyphens and the column header row is sometimes printed in a
    # doubled-character font (AATTAA DDeessccrriippttiioonn …).
    "Aircraft-Equipment-List",
    # Some operators strip the AMOS branding from the header but emit
    # the exact same column layout. The column header line itself is
