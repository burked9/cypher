"""Smoke-test the 3 new variants: On Condition Components Report,
Technical Object Listing, and OCCM List As At."""
import sys, pathlib, csv
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent.parent))
import pandas as pd
from sheet_types import occm
from sheet_types.occm_variants import (
    on_condition_components_report as occr,
    technical_object_listing as tol,
    occm_list_as_at as ola,
)

with open("research/results/triage_occm.csv") as f:
    rows = list(csv.DictReader(f))

for module in (occr, tol, ola):
